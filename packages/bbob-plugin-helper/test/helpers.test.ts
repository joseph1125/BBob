...
describe("@bbob/plugin-helper/helpers", () => {
  ...
  test("attrValue object", () => {
    ...
    expect(attrValue("test", attrs)).toBe(
      'test="{&quot;tag&quot;:&quot;test&quot;}"'
    );
  });
  ...
  describe("attrsToString escape", () => {
    test(`javascript:alert("hello")`, () => {
      expect(
        ...
        ` onclick="javascript%3Aalert(&#039;hello&#039;)" href="javascript%3Aalert(&#039;hello&#039;)"`
      );
    });
    test(`JAVASCRIPT:alert("hello")`, () => {
      expect(
  ...
  test("getUniqAttr with unq attr", () => {
    expect(getUniqAttr({ foo: true, "http://bar.com": "http://bar.com" })).toBe(
      "http://bar.com"
    );
  });
